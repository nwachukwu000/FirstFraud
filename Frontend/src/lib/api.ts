...
// Backend runs on http://localhost:5000 and https://localhost:5001
const API_BASE_URL =
...
export interface Rule {
	...
	severityWeight: number; // 0-100 percentage
	createdAt: string;
}
...
export const alertsApi = {
	...
	getTopAccounts: async (topN: number = 10): Promise<TopAccount[]> => {
		const response = await api.get<TopAccount[]>('/alerts/top-accounts', {
			params: { topN },
		});
		return response.data;
	},
...
export const rulesApi = {
	...
	toggle: async (id: string): Promise<{ isEnabled: boolean }> => {
		const response = await api.put<{ isEnabled: boolean }>(
			`/rules/${id}/toggle`
		);
		return response.data;
	},
...
export const commentsApi = {
	getByTransaction: async (transactionId: string): Promise<Comment[]> => {
		const response = await api.get<Comment[]>(
			`/comments/transaction/${transactionId}`
		);
		return response.data;
	},
...
export const auditLogsApi = {
	getList: async (params?: {
		...
		const response = await api.get<AuditLogListResponse>('/auditlogs', {
			params,
		});
		return response.data;
	},
...
export const notificationsApi = {
	getList: async (params?: {
		...
		const response = await api.get<Notification[]>('/inappnotifications', {
			params,
		});
		return response.data;
	},
