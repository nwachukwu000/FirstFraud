...
export default function RulesEngine() {
  ...
  const [ruleForm, setRuleForm] = useState({
    ...
    value: "",
    isEnabled: true,
    severity: 1, // Default to Medium
    severityWeight: 25, // Default to 25%
  });
  ...
  const updateRuleMutation = useMutation({
    mutationFn: ({ id, data }: { id: string; data: { name: string; field: string; condition: string; value: string; isEnabled?: boolean; severity?: number; severityWeight?: number } }) =>
  ...
  const handleEdit = (rule: Rule) => {
    ...
    setRuleForm({
      ...
      severityWeight: rule.severityWeight ?? 25,
    });
                ...
                >
                  <div className="flex-1 space-y-1">
                    ...
                    <div className="flex items-center gap-2 text-xs text-muted-foreground">
                      <span>Severity: {rule.severity === 0 ? "Low" : rule.severity === 1 ? "Medium" : rule.severity === 2 ? "High" : "Critical"}</span>
                      <span>•</span>
                      <span>Weight: {rule.severityWeight ?? 25}%</span>
                      <span>•</span>
                      <span>ID: {rule.id.substring(0, 8)}...</span>
                    </div>
                  </div>
              ...
              <div className="space-y-4 py-4">
                ...
                <div className="grid gap-4 md:grid-cols-2">
                  <div className="space-y-2">
                    ...
                    <Select
                      value={ruleForm.severity.toString()}
                      onValueChange={(value) =>
                        setRuleForm({ ...ruleForm, severity: parseInt(value) })
                      }
                    >
                      <SelectTrigger>
                  ...
                  <div className="space-y-2">
                    <Label htmlFor="ruleSeverityWeight">Severity Weight (%) *</Label>
                    <Input
                      id="ruleSeverityWeight"
                      type="number"
                      min="0"
                      max="100"
                      placeholder="25"
                      value={ruleForm.severityWeight}
                      onChange={(e) =>
                        setRuleForm({ ...ruleForm, severityWeight: parseInt(e.target.value) || 0 })
                      }
                      required
                    />
                    <p className="text-xs text-muted-foreground">
                      Weight in percentage (0-100) that contributes to risk score
                    </p>
                  </div>
                </div>
            ...
            <form onSubmit={handleUpdateRule}>
              <div className="space-y-4 py-4">
                ...
                <div className="grid gap-4 md:grid-cols-2">
                  ...
                  <div className="space-y-2">
                    <Label htmlFor="editRuleSeverityWeight">Severity Weight (%) *</Label>
                    <Input
                      id="editRuleSeverityWeight"
                      type="number"
                      min="0"
                      max="100"
                      placeholder="25"
